...
describe("Main Test Suite", async () => {
  describe("Dummy Token", async () => {
    it("should mint for any user", async () => {
      const { dummyToken, deployer, account1 } = await loadFixture(
        defaultFixture
      );
      ...
      expect(await dummyToken.balanceOf(account1.address)).to.equal(
        parseUnits("1000")
      );
    });
  });
  describe("JomEV Core", async () => {
    it("should add users, add provider and create station", async () => {
      const { JomEV, deployer, Vault, dummyToken, account1, account2 } =
      ...
      await approve(
        ...
        JomEV.address,
        dummyToken,
        parseUnits("0.000005").mul(24).mul(7).mul(2)
      );
      await JomEV.connect(deployer).addChargingPoint(
        parseUnits("0.000005"),
        "Damansara",
        dummyToken.address,
        2
    ...
    it("can book station", async () => {
      const { JomEV, dummyToken, Vault, deployer, account1, account2 } =
        await loadFixture(defaultFixture);
      await Vault.connect(deployer).TransferOwner(JomEV.address);
      await JomEV.connect(deployer).joinAsUser();
      await JomEV.connect(deployer).joinAsProvider();
      ...
      await JomEV.connect(deployer).bookStation(
        1,
        1,
        1,
        "0x101000",
    ...
    it("booking details are right", async () => {
      ...
      expect(stationData.availability[1].toLowerCase()).to.equal(
        output.toLowerCase()
      );
    });
    ...
    it("cannot add station if not user", async () => {
      const { JomEV, dummyToken, deployer, account1, account2 } =
        await loadFixture(defaultFixture);
      await JomEV.connect(account1).joinAsUser();
      await expect(JomEV.connect(deployer).joinAsProvider()).to.be.revertedWith(
        "This Feature is only for users"
      );
    });
    it("cannot book station if time overlaps", async () => {
      ...
      await expect(
        JomEV.connect(account1).bookStation(
          1,
          1,
          1,
          "0x100000",
      ...
      ).to.be.revertedWith("new schedule overlaps");
    });
  });
  describe("Vault", async () => {
    it("should get Correct EV Token address", async () => {
      ...
      await expect(
        await Vault.connect(deployer).getEVTokenAddress()
      ).to.be.equal(evToken.address);
    });
    it("provider should get 10% of EV Token", async () => {
      ...
      const evToken_provider = parseUnits("0.000005")
        .mul(24)
        .mul(7)
        .mul(2)
        .mul(10)
        .mul(DISTRIBUTION_POINT)
        .div(100);
      console.log("evToken_provider= ", evToken_provider);
      const deployer_vault = await Vault.connect(deployer).getEVTokenBalance(
        deployer.address
      );
    ...
    it("vault should get 90% of EV Token", async () => {
      ...
      const evToken_vault = parseUnits("0.000005")
        .mul(24)
        .mul(7)
        .mul(2)
        .mul(90)
      ...
      const vault_contract = await Vault.connect(
        deployer
      ).getEVTokenBalanceVault();
