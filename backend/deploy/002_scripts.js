...
const main = async () => {
  ...
  await withConfirmation(
    cDummyToken
      .connect(sDeployer)
      .approve(cJomEV.address, parseUnits("0.0005").mul(24).mul(8))
  );
  await withConfirmation(
    cJomEV
      .connect(sDeployer)
      .addStation(parseUnits("0.0005"), "Damansara", cDummyToken.address)
  );
  ...
  await withConfirmation(
    cDummyToken.connect(sGovernor).approve(cJomEV.address, parseUnits("0.0005"))
  );
  await withConfirmation(
    cJomEV.connect(sGovernor).bookStation(1, 1, "0x000001", cDummyToken.address)
  );
